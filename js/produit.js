(function () {
  ...
  function buildWhatsappUrl(produit) {
    var productUrl = getSiteBaseUrl() + 'produit.html?id=' + encodeURIComponent(produit.id);
    var message;

    if (produit.disponibilite === 'rupture') {
      message =
        'Bonjour Tantie Pauline, je souhaite avoir des informations sur le produit « ' + produit.nom + ' ».\n' +
        'Référence : ' + produit.id + '\n\n' +
        'Voir le produit :\n' + productUrl;
    } else {
      message =
        'Bonjour Tantie Pauline, je suis intéressé(e) par le produit « ' + produit.nom + ' » au prix de ' + formatPrice(produit.prix) + '.\n' +
        'Référence : ' + produit.id + '\n\n' +
        'Voir le produit :\n' + productUrl;
    }

    return 'https://wa.me/22600000000?text=' + encodeURIComponent(message);
  }
  ...
  function renderProduct(produit) {
    ...
    whatsappBtn.href = buildWhatsappUrl(produit);
    whatsappLabel.textContent = produit.disponibilite === 'rupture'
